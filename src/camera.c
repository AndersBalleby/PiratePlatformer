...
void boxTargetCamera(CustomCamera *camera, Player *player) {
  ...
  if (player_screen_x < camera->rect.x) {
    ...
    camera->offset.x =
        player->entity.position.x - (camera->rect.x + camera->rect.width);
  }
}
...
void drawTileWithOffset(Tile *tile, Vector2 offset) {
  ...
  DrawTexture(tile->resource->texture, (int)screen_pos.x, (int)screen_pos.y,
              WHITE);
}
...
void customDraw(CustomCamera *camera, Map *map, Player *player) {
  ...
  for (i = 0; i < ANIMATED_TILES_SIZE; ++i) {
    for (size_t j = 0; j < map->animated_tiles[i].tiles_count; ++j) {
      if (map->animated_tiles[i].anim_tiles[j].tile.active &&
          map->animated_tiles[i].anim_tiles[j].tile.resource->is_loaded)
        drawTileWithOffset(&map->animated_tiles[i].anim_tiles[j].tile,
                           camera->offset);
    }
  }
 for (i = 0; i < map->collision_tiles.tiles_size; ++i) {
    if (map->collision_tiles.tiles[i].active &&
        map->collision_tiles.tiles[i].resource->is_loaded)
      drawTileWithOffset(&map->collision_tiles.tiles[i], camera->offset);
  }

  for (i = 0; i < map->decoration_tiles.tiles_size; ++i) {
    if (map->decoration_tiles.tiles[i].active &&
        map->decoration_tiles.tiles[i].resource->is_loaded)
